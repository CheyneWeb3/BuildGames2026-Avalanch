...
The Haus Vault
  ...
  ccip avax 0xF4c7E640EdA248ef95972845a62bdC74237805dB (selector avax 6433500567565415381)
  usdt: avax 0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7
  usdc avax  0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E 
  wavax: avax  0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7

Deployed https://snowtrace.io/address/0xD72c93FE33E899f1eF4c55D718B0De8F44De516B/contract/43114/code
...
// File: HAUSV2/PHASE 1 Foundations/VaultV2.sol
...
contract TheHausVault is Pausable, EIP712, CCIPReceiver {
  ...
  bool public enforcePairAllowlist = false;
  ...
  event EnforcePairs(bool enabled);
  ...
  mapping(uint64 => bytes) public remoteVault;
  ...
  event SwapExecuted(
    ...
    address tokenOut,
    uint256 actualInUsed,
    uint256 actualOutReceived,
    uint256 maxIn,
    uint256 minOut,
    bytes4 selector,
    uint256 nonce
  ...
  bytes32 private constant SWAPCALL_TYPEHASH =
    keccak256(
      "SwapCall(address ownerWallet,address target,address tokenIn,address tokenOut,uint256 maxIn,uint256 minOut,uint256 callValue,bytes32 callHash,uint256 nonce,uint256 deadline)"
    );

  bytes32 private constant BRIDGE_USDC_TYPEHASH =
    keccak256(
      "BridgeUSDC(address ownerWallet,uint64 destSelector,address destWallet,uint256 amount,uint256 nonce,uint256 deadline)"
    );
    ...
    CCIPReceiver(_ccipRouter)
  {
    ...
    require(_ccipRouter != address(0), "BAD_CCIP");
  ...
  function setEnforcePairs(bool enabled) external onlyOwner {
    enforcePairAllowlist = enabled;
    emit EnforcePairs(enabled);
  }
  ...
  function setRemoteVault(uint64 chainSelector, address vaultAddr, bool enabled) external onlyOwner {
    remoteVault[chainSelector] = abi.encode(vaultAddr);
    remoteEnabled[chainSelector] = enabled;
    emit RemoteVaultSet(chainSelector, vaultAddr, enabled);
  }


  function depositFor(address token, uint256 amount, address creditTo) external whenNotPaused {
  ...
  function withdrawWithSig(
    ...
    emit Withdrawn(ownerWallet, token, to, amount, nonce);
  }
  ...
  function swapWithSig(
    ...
    require(block.timestamp <= deadline, "SIG_EXPIRED");
    require(msg.value == callValue, "BAD_CALL_VALUE");

    require(targetAllowed[target], "TARGET_NOT_ALLOWED");
    require(tokenConfig[tokenIn].enabled, "TOKEN_IN_DISABLED");
    require(tokenConfig[tokenOut].enabled, "TOKEN_OUT_DISABLED");

    if (enforcePairAllowlist) {
      require(pairAllowed[tokenIn][tokenOut], "PAIR_NOT_ALLOWED");
    }

    require(callData.length >= 4, "CALLDATA_SHORT");
    bytes4 selector;
    assembly {
      selector := calldataload(callData.offset)
    }
    if (selectorAllowed[bytes4(0)] == false) {
      require(selectorAllowed[selector], "SELECTOR_NOT_ALLOWED");
    }
    ...
    IERC20(tokenIn).forceApprove(target, maxIn);
    ...
    uint256 actualInUsed = inBefore - inAfter;
    uint256 actualOutReceived = outAfter - outBefore;

    require(actualInUsed <= maxIn, "IN_OVER_MAX");
    require(actualOutReceived >= minOut, "OUT_UNDER_MIN");
  ...
  function bridgeUsdcWithSig(
    ...
    uint256 deadline,
    bytes calldata sig
  ) external payable whenNotPaused onlyOperator returns (bytes32 messageId) {
    require(block.timestamp <= deadline, "SIG_EXPIRED");
    ...
    address dstVault = abi.decode(remoteVault[destSelector], (address));
    ...
    Client.EVM2AnyMessage memory m = Client.EVM2AnyMessage({
      ...
      tokenAmounts: _singletonTokenAmount(address(usdc), amount),
      feeToken: address(0), 
    ...
    emit CCIPSent(ownerWallet, destSelector, address(usdc), amount, messageId, destWallet, nonce);
  }

  function _singletonTokenAmount(address token, uint256 amount)
    internal
    pure
    returns (Client.EVMTokenAmount[] memory arr)
  {
    arr = new Client.EVMTokenAmount[](1);
  ...
  function _ccipReceive(Client.Any2EVMMessage memory message) internal override {
    ...
    address expectedSrcVault = abi.decode(remoteVault[src], (address));
    ...
    Client.EVMTokenAmount memory ta = message.destTokenAmounts[0];
